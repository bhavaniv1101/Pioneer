...
def draw_quadrilateral(
    ...
    if fill_below_bottom_edge:
        ...
        start_angle_deg = (
            LineSegment(circle_center(), bottom.start).angle() * 180 / math.pi
        )
...
def create_vanishing_point_txt(root_path, vanishing_point: Point, i_image):
    ...
    with open(f"{root_path}/vpoint_{i_image}.txt", "w") as writefile:
        writefile.write(
            f"{vanishing_point.x - SIZE / 2} {vanishing_point.y - SIZE / 2} 1"
        )


def create_vanishing_point_txt_triangle(
    root_path, tri_verts, vanishing_point: Point, i_image
):
    """Create text document with the vanishing point coordinates w.r.t. the image
    center, calculated by min-enclosing-triangle method.
    """
    # Choose triangle vertex closest to true vanishing point as the solution.
    i_v = np.argmin(
        np.sum(
            (tri_verts - np.array([vanishing_point.x, vanishing_point.y])) ** 2, axis=1
        )
    )
    with open(f"{root_path}/tri_vpoint_{i_image}.txt", "w") as writefile:
        writefile.write(
            f"{tri_verts[i_v, 0] - SIZE / 2} {tri_verts[i_v, 1] - SIZE / 2} 1"
        )
...
def create_tool_images_and_masks(root_path, num_images: int = 100, blob: bool = False):
    ...
    for i_image in range(num_images):
        ...
        # Add vertices of first quadrilateral
        this_image_vertices.append([first_bottom.start.x, first_bottom.start.y])
        this_image_vertices.append([first_bottom.end.x, first_bottom.end.y])
        this_image_vertices.append([first_top.start.x, first_top.start.y])
        this_image_vertices.append([first_top.end.x, first_top.end.y])
        ...
        draw_quadrilateral(
            draw, second_bottom, second_top, fill_below_bottom_edge=False
        )

        # Add vertices of top edge of second quadrilateral
        this_image_vertices.append([second_top.start.x, second_top.start.y])
        this_image_vertices.append([second_top.end.x, second_top.end.y])
        ...
        if random.random() > 0.5:
            ...
            draw_quadrilateral(
                draw, third_bottom, third_top, fill_below_bottom_edge=False
            )
            ...
            # Add vertices of top edge of third quadrilateral
            this_image_vertices.append([third_top.start.x, third_top.start.y])
            this_image_vertices.append([third_top.end.x, third_top.end.y])
        # if third quadrilateral is not created, use second one for vanishing point
        else:
        ...
        # Shape: (3, 2)
        # tri_verts = cv2.minEnclosingTriangle(np.array(this_image_vertices, dtype=int))[
        #     1
        # ][:, 0, :]
        tri_verts = min_enclosing_triangle_vertices(
            cv2.imread(f"{root_path}/image_{i_image}.jpg")
        )
        # print(tri_verts, vanishing_point)
        create_vanishing_point_txt_triangle(
            root_path, tri_verts, vanishing_point, i_image
        )


def min_enclosing_triangle_vertices(img: Image) -> np.ndarray:
    """Return vertices of min-area enclosing triangle."""
    ...
    boundary_points = cnts[0].reshape((-1, 2))

    return cv2.minEnclosingTriangle(boundary_points)[1][:, 0, :]


if __name__ == "__main__":
    create_tool_images_and_masks("tri_vpoint_no_blob", 20000, blob=False)
    create_tool_images_and_masks("tri_vpoint_with_blob", 20000, blob=True)
